...
@section('content')
    <x-app.page-header>
        <x-slot:pretitle>Edit</x-slot:pretitle>
        <x-slot:title>Course</x-slot:title>
    </x-app.page-header>

    <div class="page-body">
        <div class="container-xl">
            <form action="{{ route('course.update', ['course' => $course]) }}" method="post" class="row row-cards"
                            ...
                            <div class="mb-3">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                           id="title" name="title" value="{{ $course->title }}" autocomplete="off">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                @if($course->cover_image_url)
                                    <img src="{{ $course->cover_image_url }}" alt="Cover course {{ $course->title }}" class="img-fluid">
                                @endif
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="cover_image">Cover Image</label>
                                <input type="file" name="cover_image" id="cover_image" accept="image/jpeg,image/png"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-icon @error('started_at') is-invalid @enderror">
                                    ...
                                    <input class="form-control @error('started_at') is-invalid @enderror"
                                           placeholder="Select a date" id="started_at" name="started_at"
                                           value="{{ $course->started_at->format('Y-m-d H:i') }}" required>
                                </div>
                                @error('started_at')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-icon @error('ended_at') is-invalid @enderror">
                                    ...
                                    <input class="form-control @error('ended_at') is-invalid @enderror"
                                           placeholder="Select a date" id="ended_at" name="ended_at"
                                           value="{{ $course->ended_at ? $course->ended_at->format('Y-m-d H:i') : '' }}">
                                </div>
                                @error('ended_at')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
...
@push('scripts')
    ...
    <script src="https://cdn.jsdelivr.net/npm/@tabler/core@1.0.0-beta20/dist/libs/litepicker/dist/litepicker.min.js"
            defer></script>
